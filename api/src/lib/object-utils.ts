import { isISOString, parseISOString } from './date-util'
import { isHexString } from './string-utils'
...
const _parseValue = (target: any, options: ParseOptions = {}): any => {
  ...
  switch (typeof target) {
    case 'string':
      ...
      } else if (options.parseDate && isISOString(target)) {
        return parseISOString(target)
      } else {
        return target
      }
    case 'object':
